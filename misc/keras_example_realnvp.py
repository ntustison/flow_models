...
def Coupling(input_shape, hidden_layer_nodes=256, reg=0.01):
    ...
    flat_image_size = np.prod(input_shape)
    ...
    t_layer_5 = keras.layers.Dense(
        flat_image_size, activation="linear", kernel_regularizer=regularizers.l2(reg)
    ...
    s_layer_5 = keras.layers.Dense(
        flat_image_size, activation="tanh", kernel_regularizer=regularizers.l2(reg)
...
class KerasRealNVP(keras.Model):
    def __init__(self, num_coupling_layers, image_shape, hidden_layer_nodes=256, reg=0.01):
        ...
        self.layers_list = [Coupling(image_shape, hidden_layer_nodes=hidden_layer_nodes, reg=reg) for i in range(num_coupling_layers)]

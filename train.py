...
from flow_model import FlowModel  #, NegLogLikelihood
import utils
...
output_dir = "output"
model_dir = "kerasRealNVP_256x256"  # "animals_model_256x256" "model_512x512_noaug" "model_512x512_aug"
do_train = True
...
num_image_files = 16000  # num training images (ideally auto-find from directory)
...
def infinite_generator(generator):
    # Ensure train_generator repeats indefinitely (it doesn't without this - why?)
    while True:
        for batch in generator:
            yield batch
infinite_train_generator = infinite_generator(train_generator)

flow_model = FlowModel(image_shape, hidden_layers, flow_steps, validate_args)
print("")
flow_model.build(input_shape=(1,)+image_shape)
...
if do_imgs_and_points:
    ...
    training_pts, mean, reduced_cov, pca = utils.imgs_to_gaussian_pts(flow_model, train_generator, N)
    print("Now calculating Gaussian pts corresponding to first 9 'other' images...", flush=True)
    other_pts, _, _, _ = utils.imgs_to_gaussian_pts(flow_model, other_generator, 9)
    print("Now plotting 2D projection of those training points.", flush=True)
    utils.plot_gaussian_pts(training_pts, plotfile=output_dir+"/pca_training_points.png")
    print(f"Now regenerating {num_gen_images} training images...", flush=True)
    regen_pts = utils.generate_imgs_in_batches(flow_model, num_gen_images, mean, reduced_cov, pca, filename=output_dir+"/regen_image", batch_size=5, regen=training_pts)
    ...
    utils.plot_gaussian_pts(training_pts, plotfile=output_dir+"/pca_compare_points.png", mean=mean, sim_pts=sim_pts, other_pts=other_pts, num_regen=5)
